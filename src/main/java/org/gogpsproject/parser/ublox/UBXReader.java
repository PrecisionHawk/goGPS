		...
		if (data == 0x62) {
			...
			if (data == 0x02) { // RXM
				...
				if (data == 0x10) { // RAW
					...
					// RMX-RAWX
					DecodeRXMRAWX decodegnss = new DecodeRXMRAWX();
					parsed = true;
			...
			if (this.debugModeEnabled) {
				System.out.println("Warning: wrong sync char 2 " + data + " " + Integer.toHexString(data) + " [" + (
						(char) data) + "]");
			}
		}
		//	}else{
		//		//no warning, may be NMEA
		//		//System.out.println("Warning: wrong sync char 1 "+data+" "+Integer.toHexString(data)+" ["+((char)
		// data)+"]");
		//	}
		return null;
	}
