...
public class ObservationSet implements Streamable {
     ...
     *  Bit 1 set : Opposite wavelength factor to the one defined for the satellite by a previous WAVELENGTH FACT
     *  L1/2 line. Valid for the current epoch only.
	...
	public double getWavelength(int i) {
		...
		switch (this.satType) {
			...
			case 'R':
				frequency = (i == 0) ? freqNum * Constants.FR1_delta + Constants.FR1_base : freqNum * Constants
						.FR2_delta + Constants.FR2_base;
			case 'E':
	...
	public int write(DataOutputStream dos) throws IOException {
		...
		size += 1;        // 1
		// L1 data
		dos.write((byte) qualityInd[L1]);
		size += 1;
		dos.write((byte) lossLockInd[L1]);
		size += 1;
		...
		size += 1;
		if (hasL2) {
			dos.write((byte) qualityInd[L2]);
			size += 1;
			dos.write((byte) lossLockInd[L2]);
			size += 1;
