...
public class Observations implements Streamable {
	...
	public ObservationSet getSatByIDType(Integer satID, char satType) {
		if (obsSet == null || satID == null) return null;
		for (int i = 0; i < obsSet.size(); i++)
			if (obsSet.get(i) != null && obsSet.get(i).getSatID() == satID.intValue() && obsSet.get(i).getSatType()
					== satType)
				return obsSet.get(i);
		return null;
	}
...
//			if(gps.get(i)!=null && gps.get(i).getSatID()==satID.intValue() && gps.get(i).getSatType()==satType) return
// gps.get(i);
//		return null;
//	}
	...
	public void setObsSet(ArrayList<ObservationSet> obsSet) {
		this.obsSet = obsSet;
	}
	...
	public String toString() {
		...
		String out = " GPS Time:" + getRefTime().getGpsTime() + " " + sdfHeader.format(new Date(getRefTime().getMsec
				())) + " evt:" + eventFlag + lineBreak;
